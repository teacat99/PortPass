...
- **首页「到期提醒」改成卡片右上角铃铛图标**：移除原本占据整行的开关区块，改为创建规则卡片标题旁的图标按钮，PC 与移动端均节省一个垂直区段；点击切换状态、tooltip 给出当前提前分钟数与推送方式提示，权限被拒时仍以警告条显示在卡片顶部
- **规则列表 PC 端铃铛挪到操作列**：原本与端口/协议挤在一起的铃铛指示器，改为放在「续期 / 复制 / 终止」三个操作按钮的最左侧，与三者保持同一 `size-8` 视觉规格；移动端卡片不变（仍紧邻端口/协议显示）
- **首页时长快捷增加 30s 预设**：方便临时调试或验证临期推送，与原 15m / 1h / 4h / 12h / 24h 共存
- **规则页搜索框与刷新按钮加白色背景**：从透明改为 `bg-card`，与页面背景区分更清晰，提升可发现性

### 部署

```bash
docker pull teacat99/portpass:1.2.1
docker pull ghcr.io/teacat99/portpass:1.2.1
```

仅前端 UI 调整，无 API / 数据库 schema 变更，可直接平滑升级。
